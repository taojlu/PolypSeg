...
from utils.edge_erode_dataloader import get_edge_erode_loader, test_edge_erode_dataset
...
def test(model, path, dataset):
    ...
    test_loader = test_edge_erode_dataset(image_root, gt_root, 352)
    DSC = 0.0
    for i in range(num1):
...
if __name__ == '__main__':
    ...
    # criterion_loss = loss_arch.FocalLoss(apply_nonlin=torch.sigmoid, alpha=0.25, gamma=2, balance_index=0, smooth=1e-5, size_average=False)
    ...
    train_loader = get_edge_erode_loader(image_root, gt_root, batchsize=opt.batch_size, trainsize=opt.trainsize,
