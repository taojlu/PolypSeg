...
class GroupMixFormerSupervise(nn.Module):
    def __init__(
            self,
        ...
        self.norm3 = nn.BatchNorm2d(embedding_dims[3])
        self.norm2 = nn.BatchNorm2d(embedding_dims[1])
        self.norm1 = nn.BatchNorm2d(embedding_dims[0])
    ...
    def seg_head(self, x, h, w):
        ss = F.interpolate(x, size=(h, w), mode='bilinear', align_corners=False)
        return ss
    ...
    def forward(self, x):
        ...
        x1 = self.aux_head1(x1)
        x2 = self.aux_head2(x2)
        x3 = self.aux_head3(x3)
        x4 = self.aux_head4(x4)

        x1 = self.seg_head(x1, h, w)
        x2 = self.seg_head(x2, h, w)
        x3 = self.seg_head(x3, h, w)
        x4 = self.seg_head(x4, h, w)
        out = [x1, x2, x3, x4]
        # out = x1 + x2 + x3 + x4
...
if __name__ == "__main__":
    device = 'cuda'
    model = GroupMixFormerSupervise().to(device)
    model.eval()
    inputs = torch.randn(8, 3, 352, 352).to(device)
    out = model(inputs)
    print(out[0].shape)
