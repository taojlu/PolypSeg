...
class ConvStem(nn.Module):
    ...
    def __init__(self, in_dim=3, embedding_dims=64):
        ...
        self.proj2 = nn.Conv2d(mid_dim, embedding_dims, kernel_size=3, stride=1, padding=1)
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        self.gma_stage = nn.ModuleList([
            GMA_Block(
                ...
                drop=drop, attn_drop=attn_drop, drop_path_rate=drop_path_rate[i], act_layer=act_layer,
...
class GroupMixFormer(nn.Module):
    def __init__(
            ...
            num_stages=5,
            num_classes=1000,
            embedding_dims=[40, 80, 160, 320, 320],
            serial_depths=[4, 4, 4, 4, 4],
            num_heads=8,
        ...
        # self.conv = nn.Conv2d(in_channels=320, out_channels=1, kernel_size=1)
        self.conv = nn.Conv2d(in_channels=320, out_channels=1, kernel_size=1, stride=1, padding=0)
...
class UpSample(nn.Module):
    def __init__(self, in_channels, out_channels, scale_factor=2):
        ...
        self.proj = nn.Conv2d(in_channels, out_channels, kernel_size=1)  # 调整通道数
...
class DecoderPatchEmbedLayer(nn.Module):
    def __init__(self, patch_size=16, in_dim=3, embedding_dims=768, is_first_layer=False):
        super().__init__()
        if is_first_layer:
            patch_size = 1
...
class SwimGroupMixFormer(nn.Module):
    def __init__(self,  num_stages=5, embedding_dims=[40, 80, 160, 320, 320],
                 serial_depths=[2, 2, 2, 2, 2],
                 upsample_inchannel=[320,320, 240, 80],
                 upsample_outchannel=[320,160, 160, 40],
                 scale_factor=2,
                 norm_layer=nn.LayerNorm,
                 num_heads=8, mlp_ratios=[4, 4, 4, 4, 4], qkv_bias=True, qk_scale=None,
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0.2):
        super().__init__()
        self.encoders_num_stages = num_stages
        self.decoder_num_stages = num_stages - 1
        self.up_sample_inchannel = upsample_inchannel
        self.up_sample_outchannel = upsample_outchannel
        self.scale_factor = scale_factor
        self.decoder_embedding_dims = [640, 320, 240, 80, 40]

        # GroupMixFormer作为编码器
        self.encoder = GroupMixFormer(
        ...
        # Enable stochastic depth.
        dpr_per_stage = stochastic_depth(drop_path_rate, serial_depths, self.decoder_num_stages)
        # 解码器模块
        self.decoders = nn.ModuleList([
            GMA_Stage(
                dim=self.decoder_embedding_dims[i+1],
                num_heads=num_heads,
                mlp_ratio=mlp_ratios[i],
                qkv_bias=qkv_bias,
                qk_scale=qk_scale,
                ...
                norm_layer=norm_layer,
                serial_depth=serial_depths[i]
            ) for i in range(self.decoder_num_stages)
        ])

        self.decoder_patch_embed_layers = nn.ModuleList([
            DecoderPatchEmbedLayer(
                patch_size=1,
                in_dim=self.decoder_embedding_dims[i],
                embedding_dims=self.decoder_embedding_dims[i+1],
    ...
    def seg_head(self, x, h, w):

        cc = self.aux_head(x)
        # pdb.set_trace()
        ss = F.interpolate(cc, size=(h, w), mode='bilinear', align_corners=False)
        return ss
    ...
    def forward(self, x):
        ...
        for i, decoder in enumerate(self.decoders):
            # print("i: ", i)
            skip_feature = enc_features[self.encoders_num_stages - 2 - i]
            # print("skip_feature: ", skip_feature.shape)

            self.up_sample_embedding = UpSample(self.up_sample_inchannel[i], self.up_sample_outchannel[i],
                                                scale_factor=self.scale_factor).to('cuda')
            x_up = self.up_sample_embedding(x)
            x = torch.cat([skip_feature, x_up], dim=1)  # 跳跃连接
            # pdb.set_trace()
            x_patch, (H, W) = self.decoder_patch_embed_layers[i](x)
            x = self.decoders[i](x_patch, (H, W))
            x = x.reshape(b, H, W, -1).permute(0, 3, 1, 2)
...
if __name__ == "__main__":
    ...
    # 实例化模型并测试
    num_classes = 2  # 假设我们有2个类别
    model = SwimGroupMixFormer().to('cuda')
    inputs = torch.randn(1, 3, 352, 352).to('cuda')
    out = model(inputs)
    print(out.shape)  # 应该输出[1, num_classes, H, W]，其中H和W取决于输入尺寸和模型设计
