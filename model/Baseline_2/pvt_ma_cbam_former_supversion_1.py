...
class BasicConv2d(nn.Module):
    def __init__(self, in_planes, out_planes, kernel_size, stride=1, padding=0, dilation=1):
        ...
        self.conv2 = nn.Conv2d(out_planes, 1,
                              kernel_size=kernel_size, stride=stride,
                              padding=padding, dilation=dilation, bias=False)
...
class MemoryAttention(nn.Module):
    ...
    def forward(self, tgt):
        ...
        # pdb.set_trace()
        gru_out, _ = self.gru(tgt_reshaped)
        ...
        tgt = attn_output + gru_out

        tgt_reshaped = tgt.flatten(2).permute(0, 2, 1)
        gru_tgt_left, _ = self.gru(tgt_reshaped)
        gru_out = gru_tgt_left.permute(0, 2, 1).view(B, C, H, W)

        # Feedforward processing
        tgt = self.conv2(self.dropout(self.activation(self.conv1(self.norm1(tgt))))) + self.norm2(gru_out)
...
class MaCbamSuperbisionFormer_1(nn.Module):
    def __init__(self, channel=32,
        ...
        super(MaCbamSuperbisionFormer_1, self).__init__()
        ...
        # self.bridge_attention = MemoryGRUModule(channel_size=512, d_model=512, memory_size=256, num_heads=4, dropout=0.1)
    ...
    def forward(self, x):
        ...
        out4 = x.permute(0, 2, 1).view(B0, 512, 11, 11)
        ...
        out3 = x.permute(0, 2, 1).view(B0, 512, 22, 22)
        ...
        out2 = x.permute(0, 2, 1).view(B0, 512, 44, 44)
        # pdb.set_trace()
        # decoder block 1
        ...
        d2_out = F.interpolate(out2, size=(H0, W0),
                               mode='bilinear', align_corners=ALIGN_CORNERS)
        d3_out = F.interpolate(out3, size=(H0, W0),
                               mode='bilinear', align_corners=ALIGN_CORNERS)
        d4_out = F.interpolate(out4, size=(H0, W0),
                               mode='bilinear', align_corners=ALIGN_CORNERS)

        # # pdb.set_trace()
        head1 = self.head(d1_out)
        head2 = self.head(d2_out)
        head3 = self.head(d3_out)
        head4 = self.head(d4_out)


        return [head1, head2, head3, head4]


if __name__ == '__main__':
    device = torch.device('cuda:1')
    model = MaCbamSuperbisionFormer_1().to(device)
