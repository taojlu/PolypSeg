...
class PvtSkipCbam(nn.Module):
    def __init__(self, channel=32,
        ...
        super(PvtSkipCbam, self).__init__()
        ...
        self.Att3 = CBAM(576, 256)
        self.Att2 = CBAM(256, 128)
        self.Att1 = CBAM(128, 64)
    ...
    def forward(self, x):
        ...
        attention_3 = self.Att3(x)  # ([1, 256, 22, 22])
        ...
        for i, blk in enumerate(self.decoder_block3):
            x = blk(x)  # ([1, 1936, 256])
        x = self.norm3(x)
        x = self.de_block3_up(x)  # ([1, 256, 22, 22])


        # decoder block 2
        x = torch.cat((x2, x), dim=1)
        attention_2 = self.Att2(x)  # ([1, 128, 44, 44])
        ...
        attention_1 = self.Att1(x)
        x = attention_1.flatten(2).transpose(1, 2)
        # pdb.set_trace()
        for i, blk in enumerate(self.decoder_block1):
            x = blk(x)  # ([1, 7744, 64])
        x = self.norm1(x)
        x = self.de_block1_up(x)  # ([1, 64, 88, 88])


        d1_out = F.interpolate(x, size=(H0, W0),
