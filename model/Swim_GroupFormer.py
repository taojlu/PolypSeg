...
class ConvStem(nn.Module):
    ...
    def __init__(self, in_dim=3, embedding_dims=64):
        ...
        self.proj1 = nn.Conv2d(in_dim, mid_dim, kernel_size=3, stride=1, padding=1)
...
class SwimGroupFormer(nn.Module):
    def __init__(
            self,
        ...
        self.scale_factor = scale_factor
        self.decoder_embedding_dims = [640, 320, 240, 40, 40]
        ...
        # Enable stochastic depth.
        decoder_dpr_per_stage = stochastic_depth(drop_path_rate, serial_depths, self.decoder_num_stages)
        # 解码器模块
        self.decoders = nn.ModuleList([
            GMA_Stage(
                ...
                drop=drop_rate,
                attn_drop=attn_drop_rate,
                drop_path_rate=decoder_dpr_per_stage[i],
                norm_layer=norm_layer,
                serial_depth=serial_depths[i]
    ...
    def forward(self, x):
        ...
        for i, decoder in enumerate(self.decoders):
            # print('idx: ', i)
            # print("i: ", i)
            skip_feature = encoder_features[self.encoders_num_stages - 2 - i]
