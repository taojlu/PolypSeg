...
if __name__ == "__main__":
    ...
    # 创建模型，并移动到设备上
    model = UltraLight_VM_UNet(num_classes=1, input_channels=3, c_list=[8, 16, 24, 32, 48, 64], split_att='fc',
                               bridge=True).to(device)

    # 创建一个随机输入张量，假设输入的图片大小为 (1, 3, 224, 224)，并移动到设备上
    ...
    # 查看输出结果（例如前几个像素值）
    print("输出结果（前几个像素值）:", output[0, 0, :5, :5].cpu().numpy())
