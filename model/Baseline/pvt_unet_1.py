...
class PatchExpand(nn.Module):
    def __init__(self, input_resolution, input_dim, norm_layer=nn.LayerNorm, upsample_scale=2):
        super().__init__()
        self.input_resolution = input_resolution
        self.dim = input_dim
        # 修改 expand 层，使其输出通道为输入的 1/2
        self.expand = nn.Linear(self.dim, self.dim // 2, bias=False)
        self.norm = norm_layer(self.dim // 2)  # 更新 norm_layer 参数以适应新的通道数
...
class PVT_UNet_1(nn.Module):
    def __init__(self, channel=32,
                 in_channels: int = 4,
                 sr_ratios: list = [4, 2, 1, 1],
                 # input_resolution: tuple = (11,11),
        ...
        super(PVT_UNet_1, self).__init__()
        ...
        self.de_block4_up = None
        self.de_block3_up = None
        self.de_block2_up = None
    ...
    def initialize_de_block4_up(self, input_resolution, input_dim):
        self.de_block4_up = PatchExpand(input_resolution, input_dim).to(device)

    def initialize_de_block3_up(self, input_resolution, input_dim):
        self.de_block3_up = PatchExpand(input_resolution, input_dim).to(device)

    def initialize_de_block2_up(self, input_resolution, input_dim):
        self.de_block2_up = PatchExpand(input_resolution, input_dim).to(device)
    ...
    def forward(self, x):
        ...
        pvt = self.backbone(x)
        x1 = pvt[0]  # ([1, 64, 64, 64])
        x2 = pvt[1]  # ([1, 128, 32, 32])
        x3 = pvt[2]  # ([1, 320, 16, 16])
        x4 = pvt[3]  # ([1, 512, 8, 8])
        ...
        # bridge block
        bridge = self.bridge_attention(x4)  # ([1, 512, 8, 8])

        # decoder block 4

        x = bridge.flatten(2).transpose(1, 2)  # ([1, 64, 512])
        # B, N, C = x.shape
        for i, blk in enumerate(self.decoder_block4):
            x = blk(x)  # ([1, 64, 512])
        # pdb.set_trace()
        x = self.norm4(x)
        input_resolution = (H4, W4)
        input_dim = x.shape[-1]
        # pdb.set_trace()
        self.initialize_de_block4_up(input_resolution, input_dim)
        x4_up = self.de_block4_up(x).to(device)

        attention_3 = self.Att3(x3, x4_up)  # ([1, 256, 22, 22])
        ...
        input_resolution = (H3, W3)
        input_dim = x.shape[-1]
        # pdb.set_trace()
        self.initialize_de_block3_up(input_resolution, input_dim)
        x3_up = self.de_block3_up(x).to(device)

        if x3_up.size(2) == 56:
            x3_up = F.interpolate(x3_up, size=(x2.size(2), x2.size(3)), mode='bilinear', align_corners=False)

        # x3_up = self.de_block3_up(x)  # ([1, 256, 44, 44])

        # decoder block 2
        # pdb.set_trace()
        attention_2 = self.Att2(x2, x3_up)  # ([1, 256, 44, 44])
        ...
        input_resolution = (H2, W2)
        input_dim = x.shape[-1]
        # pdb.set_trace()
        self.initialize_de_block2_up(input_resolution, input_dim)
        x2_up = self.de_block2_up(x).to(device)

        # x2_up = self.de_block2_up(x)   # ([1, 256, 88, 88])
        # decoder block 1

        attention_1 = self.Att1(x1, x2_up)  # ([1, 256, 88, 88])
        ...
        # pdb.set_trace()
        d1_out = x.permute(0, 2, 1).view(B0, 512, H1, W1)
        # pdb.set_trace()
...
if __name__ == '__main__':
    ...
    model = PVT_UNet_1().to(device)
    input_tensor = torch.randn(1, 3, 440, 440).to(device)
