...
class PVT_UNet_Skip(nn.Module):
    def __init__(self, channel=32,
        ...
        super(PVT_UNet_Skip, self).__init__()
        ...
        self.de_block3_up = PatchExpand((22, 22), 256)

        self.de_block2_up = PatchExpand((44, 44), 128)

        self.de_block1_up = PatchExpand((88, 88), 64)
    ...
    def forward(self, x):
        ...
        x = x4.flatten(2).transpose(1, 2)  # ([1, 484, 512])
        x = self.de_block4_up(x)  # ([1, 256, 22, 22])
        # B, N, C = x.shape
        x = x.flatten(2).transpose(1, 2)
        ...
        # pdb.set_trace()
        x = self.norm4(x)
        d4 = x.permute(0, 2, 1).view(-1, 256, 22, 22)


        attention_3 = self.Att3(x3, d4)  # ([1, 256, 22, 22])
        x = attention_3.flatten(2).transpose(1, 2)  # ([1, 484, 256])
        x = self.de_block3_up(x)  # ([1, 128, 22, 22])
        # B, N, C = x.shape
        x = x.flatten(2).transpose(1, 2)
        for i, blk in enumerate(self.decoder_block3):
            x = blk(x)  # ([1, 1936, 128])
        x = self.norm3(x)
        d3 = x.permute(0, 2, 1).view(-1, 128, 44, 44)  # ([1, 128, 44, 44])


        # decoder block 2
        attention_2 = self.Att2(x2, d3)  # ([1, 128, 44, 44])
        x = attention_2.flatten(2).transpose(1, 2)
        x = self.de_block2_up(x)  # ([1, 64, 88, 88])
        x = x.flatten(2).transpose(1, 2)
        for i, blk in enumerate(self.decoder_block2):
            x = blk(x)
        x = self.norm2(x)
        d2 = x.permute(0, 2, 1).view(-1, 64, 88, 88)


        # decoder block 1
        attention_1 = self.Att1(x1, d2)
        x = attention_1.flatten(2).transpose(1, 2)
        # pdb.set_trace()
        for i, blk in enumerate(self.decoder_block1):
            x = blk(x)  # ([1, 7744, 64])
        x = self.norm1(x)
        d1_out = x.permute(0, 2, 1).view(B0, 64, 88, 88)
