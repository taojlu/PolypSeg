...
def calculate_total_distance(tour, distance_matrix):
    ...
    for i in range(len(tour) - 1):
        total_distance += distance_matrix[tour[i], tour[i + 1]].item()
    total_distance += distance_matrix[tour[-1], tour[0]].item()  # 回到起点的距离
...
def dijkstra(distance_matrix, start_index, device, k=6):
    """迪杰斯特拉算法进行局部优化，优先考虑当前节点周围的k个最近邻节点"""
    ...
    for _ in range(num_locations):
        ...
        # 获取当前节点u的k个最近邻节点
        neighbors = torch.argsort(distance_matrix[u])[:k]
...
def ant_colony_optimization(tour, distance_matrix, num_ants=10, alpha=1, beta=2, rho=0.5, Q=100, iterations=100, k=6):
    """蚁群算法进行局部优化，优先考虑当前节点周围的k个最近邻节点"""
    ...
    for iteration in range(iterations):
        ...
        for _ in range(num_ants):
            ...
            for i in range(1, len(tour)):
                ...
                neighbors = torch.argsort(distance_matrix[current_city])[:k]  # 获取当前节点的k个最近邻节点
                valid_neighbors = [n for n in neighbors.tolist() if n not in ant_tour]  # 过滤出未访问的邻居节点
                if not valid_neighbors:  # 如果没有未访问的邻居节点，选择任意未访问的节点
                    probabilities = pheromone[current_city] ** alpha * ((1 / distance_matrix[current_city]) ** beta)
                    probabilities[ant_tour] = 0  # 避免选择已经在路径中的城市
                    next_city = torch.multinomial(probabilities, 1).item()
                else:
                    probabilities = pheromone[current_city, valid_neighbors] ** alpha * ((1 / distance_matrix[current_city, valid_neighbors]) ** beta)
                    next_city = valid_neighbors[torch.multinomial(probabilities, 1).item()]
...
if tour_distance == float('inf'):
    ...
    parent = dijkstra(distance_matrix, start_index, device=devices[0], k=6)
    optimized_tour = reconstruct_path(parent, start_index, len(data))
    optimized_tour = ant_colony_optimization(optimized_tour, distance_matrix, k=6)
