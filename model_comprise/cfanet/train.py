...
def train(train_loader, model, optimizer, epoch, opt, loss_func, total_step):
    ...
    for step, data_pack in enumerate(train_loader):
        ...
        if step % 10 == 0 or step == total_step:
            print(
                '[{}] => [Epoch Num: {:03d}/{:03d}] => [Global Step: {:04d}/{:04d}] => [Loss_edge: {:.4f} Loss_sal1: {:0.4f} Loss_sal2: {:0.4f} Loss_sal3: {:0.4f} Loss_total: {:0.4f}]'.
                format(datetime.now(), epoch, opt.epoch, step, total_step, loss_edge.data, loss_sal1.data,
                       loss_sal2.data, loss_sal3.data, loss_total.data))
