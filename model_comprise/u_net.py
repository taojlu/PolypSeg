...
# reference: https://github.com/bigmb/Unet-Segmentation-Pytorch-Nest-of-Unets
######################################################################################
...
if __name__ == "__main__":
    ...
    # model = U_Net(in_ch=3, out_ch=1).to(device)
    model = NestedUNet(in_ch=3, out_ch=1).to(device)
    # summary(model, (3, image_size, image_size))

    in_put = torch.Tensor(4, 3, 352, 352).to(device)
    out = model(in_put)
