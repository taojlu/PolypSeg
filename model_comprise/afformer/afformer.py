...
https://github.com/dongbo811/AFFormer/tree/main
'''
import torch
...
from model_comprise.afformer.mmseg.models.builder import BACKBONES
from model_comprise.afformer.mmseg.utils import get_root_logger
...
if __name__ == '__main__':
    ...
    image_size = 352
    model = afformer_base().to(device)
    # model = R2U_Net(img_ch=1, output_ch=1, t=2).to(device)
    # summary(model, (3, image_size, image_size))

    in_put = torch.Tensor(4, 3, image_size, image_size).to(device)
    out = model(in_put).to(device)
