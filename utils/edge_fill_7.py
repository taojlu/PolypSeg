...
def select_best_image(polyp_image, edge_image, polyp_smooth, edge_smooth, polyp_coh, edge_coh, polyp_perim, edge_perim):
    smooth_diff = abs(polyp_smooth - edge_smooth) / min(polyp_smooth, edge_smooth)
    coh_diff = abs(polyp_coh - edge_coh) / min(polyp_coh, edge_coh)
    perim_diff = abs(polyp_perim - edge_perim) / min(polyp_perim, edge_perim)

    # 定义阈值
    smooth_thresh = 0.1  # 10% difference
    coh_thresh = 0.5    # 50% difference
    perim_thresh = 0.2   # 20% difference

    # 评估平滑度
    if smooth_diff > smooth_thresh:
        if polyp_smooth < edge_smooth:
            smooth_decision = "polyp"
        else:
            smooth_decision = "edge"
    else:
        smooth_decision = None

    # 评估一致性
    if coh_diff > coh_thresh:
        if polyp_coh < edge_coh:
            coh_decision = "polyp"
        else:
            coh_decision = "edge"
    else:
    ...
    # 评估周长
    if perim_diff > perim_thresh:
        if polyp_perim > edge_perim:
            perim_decision = "polyp"
        else:
            perim_decision = "edge"
    else:
        perim_decision = None
    ...
    # 最终决策逻辑
    decisions = [smooth_decision, coh_decision, perim_decision]
    final_decision = max(set(decisions), key=decisions.count)

    if final_decision == "polyp":
        return polyp_image
    elif final_decision == "edge":
        return edge_image
    else:
        # 如果无法决定（例如所有指标决策均为None），根据平滑度决策
        if smooth_decision:
            return polyp_image if smooth_decision == "polyp" else edge_image
        else:
            # 如果平滑度也未能决定，返回原始的息肉图像
            return polyp_image
...
if __name__ == '__main__':
    ...
    for num in img_list:
        ...
        # edge_contour_num = get_contour_num(edge_image)
        ...
        if len(polyp_contours) == 1:
            ...
            edge_contours = find_contours(output_image)
            print("边缘息肉轮廓数量： ", len(edge_contours))
            if len(edge_contours) == 1:
                ...
                edge_smoothness = calculate_smoothness(edge_contours[0])
                edge_perimeter = cv2.arcLength(edge_contours[0], True)

                polyp_coherence = calculate_edge_coherence(polyp_contours[0])
                edge_coherence = calculate_edge_coherence(edge_contours[0])

                print("polyp smoothness: ", polyp_smoothness)
                print("edge smoothness: ", edge_smoothness)
                print("polyp perimeter: ", polyp_perimeter)
                print("edge perimeter: ", edge_perimeter)
                print("polyp coherence: ", polyp_coherence)
                print("edge coherence: ", edge_coherence)

                # 使用示例
                edge_final = select_best_image(
                    polyp_image, edge_image,
                    polyp_smoothness, edge_smoothness,
                    polyp_coherence, edge_coherence,
                    polyp_perimeter, edge_perimeter
                )
            elif len(edge_contours) > 1:
                # max_edge = max_area_contour(output_image, edge_contours)
                # img_mask_plot(max_edge, max_edge)
            ...
            output_image = np.uint8(output_image)
            edge_contours = find_contours(output_image)
            small = max_area_contour(output_image, contours)
            #
            # pdb.set_trace()

            # img_mask_plot(edge_image, output_image)
            # edge_image[edge_image > 0] = 1
            # edge_final =  edge_image
