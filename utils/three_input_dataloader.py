...
class PolypThreeDataset(data.Dataset):
    """
    ...
    def __getitem__(self, index):
        ...
        edge_gt = get_internal_wide_edge(gt, 100)
        edge_gt = Image.fromarray(edge_gt)

        small_gt = get_small(gt)
        small_gt = Image.fromarray(small_gt)
        ...
        if self.gt_transform is not None:
            ...
            small_gt = self.gt_transform(small_gt)
        return image, gt, edge_gt, small_gt
...
def get_three_loader(image_root, gt_root, batchsize, trainsize, shuffle=True, num_workers=4, pin_memory=True,
               augmentation=True):
    dataset = PolypThreeDataset(image_root, gt_root, trainsize, augmentation)
