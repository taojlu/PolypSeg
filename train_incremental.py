...
def train(train_loader, model, criterion_loss, optimizer, epoch, test_path,
          incremental=False, old_model=None):


    scaler = GradScaler()

    if incremental and old_model is not None:
        model.load_state_dict(old_model.state_dict())  # 加载旧模型的权重
    model.train()
    global best
    ...
    for i, pack in enumerate(train_loader, start=1):
        for rate in size_rates:
            ...
            if incremental:
                # 可以在这里实施一些策略，如学习率调整、损失函数调整等，以适应新数据
...
if __name__ == '__main__':
    ...
    parser.add_argument('--incremental_learning',
                        default=True, help='use incremental learning')
    ...
    old_model = None
    if opt.incremental_learning:
        old_model = load_model(your_old_model_path)  # 加载旧模型
